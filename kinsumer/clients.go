...
func registerWithClientsTable(db dynamodbiface.DynamoDBAPI, id, name, tableName string) error {
	...
	if err != nil {
		logger.WithFields(logrus.Fields{
		  ...
		  "table": tableName,
		  "err": err.Error(),
		}).Error("error calling marshalling dynamo attribute in registerWithClientsTable")
		return err
	}
	...
	if _, err = db.PutItem(&dynamodb.PutItemInput{
		...
		logger.WithFields(logrus.Fields{
		  ...
		  "table": tableName,
		  "err": err.Error(),
		}).Error("error registering in clients table")
		return err
	}
	...
	logger.WithFields(logrus.Fields{
		...
		"name": name,
		"table": tableName,
	}).Info("Client registered in dynamo")
...
func deregisterFromClientsTable(db dynamodbiface.DynamoDBAPI, id, tableName string) error {
	...
	if _, err = db.DeleteItem(&dynamodb.DeleteItemInput{
		...
		logger.WithFields(logrus.Fields{
		  ...
		  "table": tableName,
		  "err": err.Error(),
		}).Error("error deregistering from clients table")
		return err
	}
	...
	logger.WithFields(logrus.Fields{
	    "id": id,
		"table": tableName,
	}).Info("Client deregistered in dynamo")
...
func getClients(db dynamodbiface.DynamoDBAPI, name string, tableName string, maxAgeForClientRecord time.Duration) (clients []clientRecord, err error) {
	...
	if innerError != nil {
		logger.WithFields(logrus.Fields{
		  ...
		  "name": name,
		  "err": err.Error(),
		}).Error("error calling unmarshalling dynamo attribute in getClients")
...
func reapClients(db dynamodbiface.DynamoDBAPI, tableName string) error {
	...
	if innerError != nil {
		logger.WithFields(logrus.Fields{
		  "table": tableName,
		  "err": err.Error(),
		}).Error("error calling unmarshalling dynamo attribute in reapClients")
		return innerError
	}
	...
	for _, client := range clients {
		...
		if err != nil {
			logger.WithFields(logrus.Fields{
			  ...
			  "clientId": client.ID,
			  "err": err.Error(),
			}).Error("error calling marshalling dynamo attribute in reapClients")
			return err
		}
		if _, err = db.DeleteItem(&dynamodb.DeleteItemInput{
			TableName:           aws.String(tableName),
			...
			logger.WithFields(logrus.Fields{
			  ...
			  "clientId": client.ID,
			  "err": err.Error(),
			}).Error("error removing client from dynamo")
			return err
		}
	}
	...
	logger.WithFields(logrus.Fields{
		"table": tableName,
	}).Info("Old clients successfully reaped")
