...
func (s *Statsd) EmitEventsMetrics(emitName string, numEvents int, processTime time.Duration) {
	_ = s.client.Inc(fmt.Sprintf("kinsumer.%s_events", emitName), int64(numEvents), 1.0)
	_ = s.client.TimingDuration(fmt.Sprintf("kinsumer.%s_time", emitName), processTime, 1.0)
}

func (s *Statsd) BatchEventsMetrics(batchName string, batchSize int64, batchTime time.Duration) {
	fmt.Println(batchName, batchSize, batchTime)
	_ = s.client.Inc(fmt.Sprintf("kinsumer.%s_batchsize", batchName), batchSize, 1.0)
	_ = s.client.TimingDuration(fmt.Sprintf("kinsumer.%s_batchtime", batchName), batchTime, 1.0)
}
